...
### 新冠后遗症舆情数据集
本数据源自2019级信息资源管理专业本科毕业论文《网络社交媒体中的科学极化现象——以新冠肺炎后遗症为例》，毕业生张天题；以及2020级信息资源管理专业本科毕业论文《网络社区用户群体极化的类型演变现象研究：以对新冠后遗症的讨论为例》，毕业生董展辰。
#### 数据集背景
1. **研究主题**：以“新冠”和“后遗症”为核心，研究公众在不同网络平台上的关注与讨论，分析公众观点分布、极化现象和传播机制。
2. **数据来源**：
   - 第一部分：Bilibili平台的视频评论。
   - 第二部分：新浪微博的推文数据。研究选择2022年12月1日至2023年3月1日期间的微博内容，以捕捉在该时间范围内观点极化形成的动态。
3. **数据采集说明**：
   - **Bilibili评论数据**：使用EasySpider软件采集；
   - **微博数据**：
      - 关键词检索，时间范围为2022年12月1日至2023年3月1日。
      - 选取相关关键词后筛选出讨论量最高的内容。
      - 按转发、点赞、评论数降序排列，选取排名前1000的微博。
      - 从1000篇文章中选取150位不重复的博主。- 包括官方媒体/组织：12位，微博认证用户：55位，普通用户：83位。

#### 数据集的数量
**Bilibili评论数据**
1. **原始数据量**：51,807条评论。
2. **筛选后数据量**：7,989条与“新冠后遗症”相关的评论。
3. **数据处理与筛选**：
   - 使用BERT模型（bert-base-chinese）筛选相关评论。
   - 随机抽取2000条评论进行人工标注，用于模型微调（9:1划分训练集与测试集）。
   - 筛选效果：人工检验100条，相关性96%。

**新浪微博数据**
1. **初步数据量**：筛选出讨论量最高的1000篇微博内容。
2. **用户数据**：
   - 包括150位用户（官方媒体/组织：12人；认证用户：55人；普通用户：83人）。
3. **用户态度分类**：
   - 中立/客观态度：34人。
   - 认为后遗症严重：67人。
   - 认为影响轻微：49人。
